...
def score_case(fl, t_s, soft=P_SOFTFP, bw=None):
    ach = fl["total"] / t_s
    geo = fl["gemm"] / P_INT + (fl["total"] - fl["gemm"]) / soft
    mix_pk = fl["total"] / geo
    mfu_mix = ach / mix_pk
    mfu_int = ach / P_INT
    # note: mfu_mix == roof_t/t_s when the fp32 part is compute-bound (it is)
    ai_total = fl["total"] / (bytes_gemm_forward(fl) + bytes_aux_forward(fl))
    # roofline per class, then aggregate
    ...
    return {"t_s": t_s, "achieved_mflop_s": ach / 1e6,
...
def main():
    ...
    for c, w in zip(cases, wts):
        r = score_case(fl, c["t_s"], bw=BW_HI)
        r["case"], r["w"] = c["case"], w
        rows.append(r)
