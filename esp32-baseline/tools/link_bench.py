...
def parse_feed(chunks):
    ...
    summary = {"boot": {}, "rounds": [], "server_last": None, "done": False}
...
def summarize(parsed):
    ...
    if not parsed["rounds"]:
        ...
        return "\n".join(lines)
    tot = 0.0
    lines.append("  payload  sent  fail  err  acked  loss%  rtt_us_med  (KB/s  MB/s)")
    for r in parsed["rounds"]:
        ...
        lines.append("  %6d  %4d  %4d  %3d  %5d   %5.1f   %9s   %6.1f %.3f" % (
            r["P"], r["sent"], r["fail"], r["err"], r["acked"], loss,
...
def main():
    ...
    ps = parse_feed(buf_s)
    if parsed["boot"]:
        parser_server_mac = parsed["boot"].get("SERVER") or ps["boot"].get("SERVER")
        # boot line may land on either serial; merge
