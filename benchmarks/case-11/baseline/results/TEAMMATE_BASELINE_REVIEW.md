...
independently recomputed seed 0 through the vendored torch_ref, built
...
| Worst C FAST absolute error | 0.0011 |
...
5. At `H=16` head width is 8, so per-head QK/PV rows are tiny (128x8)
   ...
   dot-product-bound. The 2.460 s forward is dominated by the full-D
   projections (identical to case-2 per input) plus per-head loop
   overhead.
6. This is the official benchmark's Transformer body, not yet a trained
...
may rival the dot-product work. The measured 2.460 s single-input is
