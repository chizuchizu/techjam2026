...
| Per-board compute | `[119.094, 119.101]` s |
| Cluster compute wall | **119.101 s** |
| Equivalent one tiled worker | 238.195 s |
| Representative forward | 3.7216 s |
| Compute scaling vs one tiled worker | **2.00x** |
| Compute gain vs best optimized single board | **1.16x** |
| End-to-end wall including WiFi TCP | **146.7 s** |
...
one forward from about 2.165 s to 3.722 s. Two replicas scale the tiled path
perfectly, while the memory-saving schedule limits the gain against the
fastest non-WiFi firmware.

Raw result: [`results_case10_two_c3_wifi.json`](results_case10_two_c3_wifi.json).
...
.venv/bin/python benchmarks/batch-dp/tools/run_batch_dp.py \
  ...
  --json benchmarks/case-10/multiboard/results_case10_two_c3_wifi.json
```
