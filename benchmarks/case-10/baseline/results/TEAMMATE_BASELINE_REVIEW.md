...
parameters (two heads of width 64). Host side is fully verified; device
side fails to link and is documented as an SRAM anomaly.

| Check | Independent result |
|---|---:|
| C FAST, 25 host seeds | 0 / 409,600 failed outputs |
| Worst C FAST absolute error | 0.001089 |
| C EXACT, 25 host seeds | 0 / 409,600 failed outputs |
| Worst C EXACT absolute error | 0.000091 |
...
2. This is **not** a host-gate or accuracy failure. The regenerated C
   host passes 50/50 seed-runs (25 x FAST + EXACT); regeneration and
   rerun are deterministic.
...
5. `weights.bin`/`weights_q12.bin`/`testdata/` and `.pio/` are
   gitignored (regenerable). Any collaborator can reproduce host PASS
   with the committed steps.
