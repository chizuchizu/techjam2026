# Review of `techjam2026/case-05/optimisation/esp32-baseline`
...
2026-08-29. The case-05 baseline directory is committed with this
review; see `git log` for the case-05 commit.

## What is confirmed

The teammate implementation is the benchmark-shaped Transformer body at
`B=128, S=128, D=128, H=4, F=128, L=4`, causal, with 398,592
...
is
[`case-05_esp32_baseline_seed0_v1.log`](case-05_esp32_baseline_seed0_v1.log).
...
| Case 5 complete forward (128 x 1.996 s) | 255.488 s |

The numerical result is confirmed on host. The device timing for this
case is not a fresh physical capture: case-05 belongs to the batch
...
2. `weights.bin`, `weights_q12.bin`, and `testdata/` are intentionally
   ...
   (`python3 tools/export_case2.py --outdir . --seeds 25 --B 128 --S 128 --D 128 --H 4 --F 128 --L 4`)
...
4. The firmware streams one input frame per forward; a batch of 128
   inputs is 128 sequential forwards with resident weights. Batch time
...
Keep this large baseline as the batch-128 correctness target. The
...
multiboard dispatch of the 128 independent inputs is the better
