...
def usbipd_busids():
    """Discover the boards' usbip bus ids. They are not stable: flashing resets
    the board's USB and replugging moves it to a different bus."""
    ...
    for line in out.splitlines():
        ...
        if line.startswith("Persisted:"):
            connected = False
        if connected and "JTAG/serial debug unit" in line:
...
def usbipd_reattach(busids):
    ...
    busids = usbipd_busids() or busids
...
def main():
    ...
    ap.add_argument("--usbipd-busids", default="",
                    help="WSL usbip bus ids to re-attach when a port stalls; "
                         "discovered automatically when empty, 'none' disables")
    args = ap.parse_args()
    ...
    Node.busids = ([] if args.usbipd_busids == "none"
                   else [b for b in args.usbipd_busids.split(",") if b]
                        or usbipd_busids())
    if Node.busids:
        print(f"[usbip] boards on bus {', '.join(Node.busids)}")
