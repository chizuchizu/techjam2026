...
|---|---:|---:|---:|---|
| Optimised single-board firmware (opt23) | 1 C3 | 1.990 s | 1.00x | 5/5 seeds |
| This cluster | 2 C3s | **1.276 s** | **1.56x** | 25/25 host + device seeds, 0 failing elements |

Both rows are the same two physical boards at 160 MHz, same weights, same
kernels. The cluster tracks the maintained single-board FAST path: int32
fixed-point residual stream, LayerNorm fused into the Q15 quantize, Q15
attention context at a global scale, and the O/FFN2 projections folded
straight into the residual. Full numbers and the derivation of the partition are in
...
throughout. Measured time actually spent *waiting* on the link is **5–88 ms per
...
## Layout
    ...
    tools/time_cluster.py   timing without moving a 32 KB frame
...
25/25 seeds pass, 0 failing elements, worst max_abs 1.02e-3.
...
The tooling therefore discovers usbip bus ids (they move when a board is
flashed or replugged), recycles the handle, re-attaches, re-resolves which tty
belongs to which node, and falls back to the board's own record of its last
forward when the reply is lost — `O` re-streams the output on demand.
`time_cluster.py` exists for the same reason: it measures without moving a
single 32 KB frame. A native Linux or macOS host does not need any of this.
