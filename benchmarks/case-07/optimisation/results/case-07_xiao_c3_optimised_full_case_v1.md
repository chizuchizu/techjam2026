# Case 07 — optimised ESP32-C3 full-case on-board result (v1, measured)
...
- Worst per-element absolute error over all 64 frames: **1.5579e-03**
- **Total on-device time for the full case: 30.4272 s**
  (sum of the firmware `us=` forward counters over all 64 frames)
- Median per-forward time: **0.475334 s** (context; the headline
  number above is the complete-case total)
- **Wall time for the full case (flash/upload/verify excluded):
  70.227 s** (first frame send -> last frame output, includes host
...
Raw capture: `case-07_xiao_c3_optimised_full_case_v1.log` (64/64 frames with per-frame fails/max_abs/us).
