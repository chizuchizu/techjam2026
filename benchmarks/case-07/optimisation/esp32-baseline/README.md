...
* **FAST** (`TM_MODE_FAST`, default): Q15 activations and Q12 weights feed
  ...
  and biases are pre-quantized offline. **Measured on device: 0.491 s/forward**
...
(FAST worst 1.3875e-03; EXACT worst 6.71e-05). On-device: **5/5 seeds pass**,
worst max_abs 1.4284e-03, 0.491 s/forward.
...
Param count 25,664 = 0.10 MB fp32. XIAO build RAM 83,332 B (fits 400 KB).
App partition 418,610 B (fits 4 MB). Host-validated accuracy: FAST
0/25 seed failures (worst max_abs 1.3875e-03); EXACT 0/25 (worst 6.71e-05).

## Current measured execution (on-device, FAST mode)

Device gate (FAST, 5 seeds): **5/5 PASS**, max_abs up to 1.4284e-03.
On-device forward time is **0.491 s/forward** (measured on board A).
...
Model work per forward (2 FLOP/MAC), **11.77 MFLOP**:
- **6.29 M** int16 GEMMs (q,k,v,o,f1,f2 projections), FAST path;
- **4.23 M** fp32 causal attention (QK^T + PV, dequant-on-read);
