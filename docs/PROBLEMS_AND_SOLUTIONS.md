...
| More boards introduce stragglers and failures. | A dual-core ESP32 finishes a head about 14x faster than the C3, while two matched C3 boards differ by only 0.2%. | Measured scheduling gives a heterogeneous 1+3 split and a matched 2+2 split; the matched pair reaches 1.98–2.00x speedup. | Cache profiles, add deadlines/retries, and scale the matched pool to four boards. |
